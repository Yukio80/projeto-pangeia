...
_RESOURCE_DEFS: list[tuple[str, str, str]] = [
    ...
    ("pangeia://history",            "Aggregated history of past analyst reports",      "Previous analyst reports for longitudinal reasoning"),
    ("pangeia://snapshots",          "Full state snapshots available on disk",          "List of snapshot metadados: tick, filename, size, timestamp, label"),
]
...
async def handle_read_resource(uri: str) -> list[TextResourceContents]:
    ...
    if uri_str in _RESOURCE_URI_SET:
        fetch_fn = _RESOURCE_FETCHERS.get(uri_str)
    elif uri_str.startswith("pangeia://timeline/"):
        metric = uri_str[len("pangeia://timeline/"):]
        fetch_fn = lambda m=metric: _fetch_timeline(m)
    else:
    ...
    try:
        if uri_str not in ("pangeia://history", "pangeia://snapshots") and not uri_str.startswith("pangeia://timeline/"):
        ...
        if uri_str in ("pangeia://history", "pangeia://snapshots") or uri_str.startswith("pangeia://timeline/"):
            text = await fetch_fn()
        else:
            text = await cached_get(uri_str, fetch_fn)
        ...
        text = _error_json(str(e), uri_str)
    except Exception as e:
        text = _error_json(f"Unexpected error: {e}", uri_str)
...
async def _fetch_history() -> str:
    ...
    if not reports_dir.is_dir():
        return _fmt_json({
            "total_reports": 0,
            "reports": [],
            "message": "Nenhum relatório encontrado. Use o Pangeia Analyst para gerar relatórios.",
        })

    files = sorted(reports_dir.glob("*.md"), key=lambda f: f.name, reverse=True)
    reports_list = []
    for f in files:
        try:
...
async def _fetch_snapshots() -> str:
    from pathlib import Path
    snap_dir = Path("snapshots")
    if not snap_dir.is_dir():
        return _fmt_json({
            "error": "No snapshots found",
            "hint": "Use POST /snapshot to create one",
        })
    files = sorted(snap_dir.glob("*.json"), key=lambda f: f.name)
    snapshots = []
    for f in files:
        try:
    ...
    return _fmt_json({"snapshots": snapshots, "total": len(snapshots)})


async def _fetch_timeline(metric: str) -> str:
    valid = ["gdp", "population", "stability", "inequality",
             "happiness", "technology_count", "volatility"]
    if metric not in valid:
        return _fmt_json({
            "error": f"Unknown metric '{metric}'",
            "valid_metrics": valid,
        })
    data = await _api_get(f"/timeline?metric={metric}")
    return _fmt_json({
...
_RESOURCE_FETCHERS: dict[str, Any] = {
    ...
    "pangeia://history": _fetch_history,
    "pangeia://snapshots": _fetch_snapshots,
}
